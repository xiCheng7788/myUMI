...
const getName = (absPath, srcDirPath) => {
  const suffix = /(\.model)?\.(j|t)sx?$/;
  const dir = srcDirPath === null || srcDirPath === void 0 ? void 0 : srcDirPath.find(dir => absPath.includes(dir));

  if (dir) {
    const name = absPath.replace(dir, '').split(_path().default.sep).join('.').replace(/(models|model)\./g, '').replace(suffix, '');
    return name.startsWith('.') ? name.slice(1) : name;
  }

  return _path().default.basename(absPath).replace(suffix, '');
};
...
const genExtraModels = (models = []) => models.map(ele => {
...
const genModels = (imports, srcDirPath) => {
