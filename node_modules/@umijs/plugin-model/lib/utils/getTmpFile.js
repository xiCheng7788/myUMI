"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.getTmpFile = void 0;

function _react() {
  const data = _interopRequireDefault(require("react"));

  _react = function _react() {
    return data;
  };

  return data;
}

var _getProviderContent = _interopRequireDefault(require("./getProviderContent"));

var _getUseModelContent = _interopRequireDefault(require("./getUseModelContent"));

function _os() {
  const data = require("os");

  _os = function _os() {
    return data;
  };

  return data;
}

function _umi() {
  const data = require("umi");

  _umi = function _umi() {
    return data;
  };

  return data;
}

var _ = require(".");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const winPath = _umi().utils.winPath;

function getModels(files, srcDirPath) {
  const sortedModels = (0, _.genModels)(files, srcDirPath);
  return sortedModels.map(ele => `'${ele.namespace.replace(/'/g, "\\'")}': ${ele.importName}`).join(', ');
}

function getExtraModels(models = []) {
  const extraModels = (0, _.genExtraModels)(models);
  return extraModels.map(ele => `'${ele.namespace}': ${winPath(ele.importName)}`).join(', ');
}

function getExtraImports(models = []) {
  const extraModels = (0, _.genExtraModels)(models);
  return extraModels.map(ele => `import ${ele.importName} from '${winPath(ele.importPath.replace(/'/g, "\\'"))}';`).join(_os().EOL);
}

const getTmpFile = (files, extra = [], srcDirPath) => {
  const imports = (0, _.genImports)(files);
  const userModels = getModels(files, srcDirPath);
  const extraImports = getExtraImports(extra);
  const extraModels = getExtraModels(extra);
  const enable = Boolean(imports || extraImports);
  return {
    providerContent: (0, _getProviderContent.default)(imports, userModels, extraImports, extraModels),
    useModelContent: enable ? (0, _getUseModelContent.default)() : 'export const useModel = undefined;'
  };
};

exports.getTmpFile = getTmpFile;